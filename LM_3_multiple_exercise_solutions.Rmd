...
10\. Let's plot the predictions of your initial model to figure out how it really fits the data. Here's a general recipe, using the `predict()` function.
...
[Optional] Alternative method using less code:
...
Let's now explore this question left unanswered: "is it okay to assume the effect of `LOGAREA` to be the same for all grazing levels?" This is effectively asking if we should let the slope of `LOGAREA` vary for different `FGRAZE` levels, which is the definition of an interactive effect.

\  

12\. Fit a linear model in R to explain the variation in the response variable, `ABUND` with the explanatory variables `LOGAREA` and `FGRAZE` and the interaction between these variables. Hint: this time, the interaction is included using the `*` symbol instead of `+`. Remember to use the `data =` argument. Assign this linear model to an appropriately named object, like `birds.inter.1`.
